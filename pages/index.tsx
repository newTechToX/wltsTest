...
const KitchenSink: NextPage = () => {
  ...
  const connect = async () => {
    ...
    const newChain = (await provider.getNetwork()).chainId;
    ...
    <div className="flex min-h-screen flex-col items-center justify-center py-2">
      <main className="flex w-full flex-1 flex-col items-center justify-start px-20 text-center gap-2">
        <button className="border border-black p-2" onClick={connect}>
          {chainId && address ? `${chainId} - ${address}` : 'Connect'}
        </button>

        {address && chainId && <MethodToggle method={method} setMethod={setMethod} />}
        ...
        {address && chainId && (
          <>
            ...
            <FixtureCategory title="Permit">
              <SingleFixture title="Permit" fixture={new PermitFixture(address, chainId)} method={method} />
              <SingleFixture title="Permit2 Single" fixture={new Permit2SingleFixture(address, chainId)} method={method} />
              <SingleFixture title="Permit2 Batch" fixture={new Permit2BatchFixture(address, chainId)} method={method} />
              <SingleFixture title="PermitForAll" fixture={new PermitForAllFixture(address, chainId)} method={method} />
              <SingleFixture title="Permit (DAI + eth_signTypedData_v3)" fixture={new DaiPermitFixture(address, chainId)} method={method} />
              <SingleFixture title="Permit (decimal address)" fixture={new PermitDecimalAddressFixture(address, chainId)} method={method} />
            </FixtureCategory>

            <FixtureCategory title="NFT Listings">
              <SingleFixture title="Seaport v1" fixture={new Seaport1Fixture(address, chainId)} method={method} />
              <SingleFixture title="Seaport v1 (empty consideration)" fixture={new Seaport1Fixture(address, chainId, [])} method={method} />
              <SingleFixture title="Seaport v1.4 (bulk)" fixture={new Seaport14Fixture(address, chainId)} method={method} />
              <SingleFixture title="LooksRare" fixture={new LooksRareFixture(address, chainId)} method={method} />
              <SingleFixture title="Blur" fixture={new BlurFixture(address, chainId)} method={method} />
              <SingleFixture title="Blur (bulk)" fixture={new BlurBulkFixture(address, chainId)} method={method} />
            </FixtureCategory>
            ...
            <FixtureCategory title="Mutated Data">
              <SingleFixture title="inconsistent chainId" fixture={new InconsistentChainID(address)} method={method} />
              <SingleFixture title="numerical address" fixture={new NumericalAddr(address, chainId)} method={method} />
              <SingleFixture title="inconsistent website" fixture={new InconsistentWebsite(address, chainId)} method={method} />
            </FixtureCategory>
